...
(function($) {
    ...
    window.PDFManagerDocuments = {
        ...
        // Process image URLs in content to use proxy
        processImageUrls: function(content) {
            if (!content) return content;
            
            // Regular expression to find image URLs with :8001/image pattern
            const imageUrlPattern = /(https?:\/\/[^:]+):8001\/image([^"'\s>]+)/g;
            
            // Replace image URLs with proxy URLs
            content = content.replace(imageUrlPattern, (match, baseUrl, imagePath) => {
                // Create proxy URL
                const proxyUrl = this.createProxyImageUrl(baseUrl + ':8001/image' + imagePath);
                console.log('🖼️ Converting image URL:', match, '→', proxyUrl);
                return proxyUrl;
            });
            
            // Also handle img tags with src attributes
            content = content.replace(/<img([^>]+)src=["']([^"']+:8001\/image[^"']+)["']([^>]*)>/gi, (match, before, imageUrl, after) => {
        ...
        // Update selected count in tag manage modal
                ...
                if (data.status === "success") {
                    ...
                    const html = sections.map(
                        s => {
                            ...
                            let processedContent = window.PDFManagerUtils.processContent(s.content || '');
                            // Apply image URL proxy processing
                            processedContent = this.processImageUrls(processedContent);
        ...
        openTagRenameModal: function(oldTagName) {
            ...
            // Set old tag name display
            oldNameEl.textContent = oldTagName;
            
            // Set input value to current tag name
