// Facility Definition Frontend Admin JavaScript
(function($) {
    'use strict';
    ...
    function bindEvents() {
        // 탭 네비게이션
        $('.fd-fa-tab-item').on('click', function(e) {
            e.preventDefault();
            var $this = $(this);
            if (!$this.hasClass('active')) {
                $('.fd-fa-tab-item').removeClass('active');
        ...
        $('#clear-all-data').on('click', function() {
            if (confirm(fd_frontend.messages.clear_confirm)) {
                clearAllData();
            }
        });
    ...
    function switchSection(section) {
        ...
        // 모든 섹션 숨기기
        $('.fd-fa-section').removeClass('active');
        
        // 선택된 섹션 표시
        $('.fd-fa-section[data-section="' + section + '"]').addClass('active');
    }
    ...
    function clearAllData() {
        ...
        var $button = $('#clear-all-data');
        $button.prop('disabled', true).addClass('fd-fa-loading');
        
        $.ajax({
            url: fd_frontend.ajax_url,
            type: 'POST',
            data: {
                action: 'fd_frontend_clear_data',
                nonce: fd_frontend.nonce
            },
            success: function(response) {
                if (response.success) {
                    ...
                    alert(response.data || fd_frontend.messages.save_error);
                }
            },
            error: function() {
                alert(fd_frontend.messages.save_error);
            },
            complete: function() {
                state.isLoading = false;
                $button.prop('disabled', false).removeClass('fd-fa-loading');
            }
        });
    }
    ...
    function saveBackgroundSettings() {
        ...
        var settings = {
            action: 'fd_frontend_save_background',
            nonce: fd_frontend.nonce,
